...
def fix_init_all_vars(sess):
    ...
    for i in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES):
        agn.append(tf.compat.v1.assign(i, tf.compat.v1.random_uniform(tf.shape(i), minval=0.01, maxval=0.1, seed=1, dtype=i.dtype.base_dtype)))
    sess.run(agn)
...
def load_model(sess, model_path, load_meta_graph=False):
    ...
    if load_meta_graph:
        tf.compat.v1.train.import_meta_graph(model_path + 'model.cptk.meta')
