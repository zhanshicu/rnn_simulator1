...
def mmd_loss(source_samples, target_samples, weight, scope=None):
  ...
  with tf.control_dependencies([assert_op]):
    ...
    tf.compat.v1.losses.add_loss(loss_value)
...
def correlation_loss(source_samples, target_samples, weight, scope=None):
  ...
  with tf.control_dependencies([assert_op]):
    ...
    tf.compat.v1.losses.add_loss(corr_loss)
...
def dann_loss(source_samples, target_samples, weight, scope=None):
  ...
  with tf.compat.v1.variable_scope('dann'):
    ...
    # Replace slim.fully_connected with tf.keras.layers.Dense or tf.compat.v1.layers.dense
    grl = tf.compat.v1.layers.dense(grl, 100, activation=tf.nn.relu, name='fc1')
    logits = tf.compat.v1.layers.dense(grl, 1, activation=None, name='fc2')

    domain_predictions = tf.sigmoid(logits)

  domain_loss = tf.compat.v1.losses.log_loss(
